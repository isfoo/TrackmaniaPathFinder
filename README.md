...
Once you provide the input you can click **Run default algorithm** and it's going to generate the solutions in table below. Once it's completed you will see **Status** change to **Done**. The solutions found are guaranteed to be optimal top **max nr of routes** solutions. In case you provide more than 1 ring CP **Run default algorithm** will not be able to find the solutions that contain a sequence of 2+ ring CPs followed by a respawn. In that case if there is a chance such paths can happen in optimal routes you should use **Run node-by-node algorithm** instead.
...
Most of the new options should be understable upon reading the **(?)** tooltip text. In this section I will explain the usage of other algorithms and you can read about the [connection finder mode](#connection-finder-mode) below.

There are 4 algorithms you can use:
- **default** - 100% guarantee of solution optimality, much faster than **node-by-node** and **arborescence**, however only has partial ring CP support (no 2+ ring CPs followed by respawn)
- **node-by-node** - 100% guarantee of solution optimality and full ring CP support, but in practice probably only handles maps up to around 25 CPs.
- **arborescence** - Same properties as **node-by-node**, but depending on input it can be faster or slower.
- **heuristic** - No guarantee on solution quality, however in practice based on experience it should find most if not all top solutions.

If you have no ring CPs you should always prefer **default** algorithm and if you have ring CPs that can happen to chain before respawn you should prefer **node-by-node** or **arborescence** depending on whichever happens to be faster for your input. However if after trying running the chosen algorithm it is not completing fast enough here are the recommended steps you should try in order:
