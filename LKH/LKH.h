...
/*
    Added by @isfoo
    Dirty hack to fix even dirtier code.
    Gain23 function uses static variable that could hold invalid pointer across runs.
    This global variable "fixes" it
*/
extern int Gain23NeedToReset;
...
extern int Distance_1(Node * Na, Node * Nb);
extern int Distance_LARGE(Node * Na, Node * Nb);
extern int Distance_ATSP(Node * Na, Node * Nb);
extern int Distance_ATT(Node * Na, Node * Nb);
extern int Distance_CEIL_2D(Node * Na, Node * Nb);
extern int Distance_CEIL_3D(Node * Na, Node * Nb);
extern int Distance_EXPLICIT(Node * Na, Node * Nb);
extern int Distance_EUC_2D(Node * Na, Node * Nb);
extern int Distance_EUC_3D(Node * Na, Node * Nb);
extern int Distance_FLOOR_2D(Node * Na, Node * Nb);
extern int Distance_FLOOR_3D(Node * Na, Node * Nb);
extern int Distance_GEO(Node * Na, Node * Nb);
extern int Distance_GEOM(Node * Na, Node * Nb);
extern int Distance_GEO_MEEUS(Node * Na, Node * Nb);
extern int Distance_GEOM_MEEUS(Node * Na, Node * Nb);
extern int Distance_MAN_2D(Node * Na, Node * Nb);
extern int Distance_MAN_3D(Node * Na, Node * Nb);
extern int Distance_MAX_2D(Node * Na, Node * Nb);
extern int Distance_MAX_3D(Node * Na, Node * Nb);
extern int Distance_SPECIAL(Node * Na, Node * Nb);
extern int Distance_TOR_2D(Node * Na, Node * Nb);
extern int Distance_TOR_3D(Node * Na, Node * Nb);
extern int Distance_XRAY1(Node * Na, Node * Nb);
extern int Distance_XRAY2(Node * Na, Node * Nb);

extern int D_EXPLICIT(Node * Na, Node * Nb);
extern int D_FUNCTION(Node * Na, Node * Nb);

extern int C_EXPLICIT(Node * Na, Node * Nb);
extern int C_FUNCTION(Node * Na, Node * Nb);

extern int c_ATT(Node * Na, Node *Nb);
extern int c_CEIL_2D(Node * Na, Node * Nb);
extern int c_CEIL_3D(Node * Na, Node * Nb);
extern int c_EUC_2D(Node * Na, Node * Nb);
extern int c_EUC_3D(Node * Na, Node * Nb);
extern int c_FLOOR_2D(Node * Na, Node * Nb);
extern int c_FLOOR_3D(Node * Na, Node * Nb);
extern int c_GEO(Node * Na, Node * Nb);
extern int c_GEOM(Node * Na, Node * Nb);
extern int c_GEO_MEEUS(Node * Na, Node * Nb);
extern int c_GEOM_MEEUS(Node * Na, Node * Nb);

extern void Activate(Node * t);
extern int AddCandidate(Node * From, Node * To, int Cost, int Alpha);
extern void AddExtraCandidates(int K, int CandidateSetType, int Symmetric);
extern void AddTourCandidates(void);
extern void AdjustCandidateSet(void);
extern void AdjustClusters(int K, Node ** Center);
extern void AllocateSegments(void);
extern void AllocateStructures(void);
extern GainType Ascent(void);
extern Node *Best2OptMove(Node * t1, Node * t2, GainType * G0, GainType * Gain);
extern Node *Best3OptMove(Node * t1, Node * t2, GainType * G0, GainType * Gain);
extern Node *Best4OptMove(Node * t1, Node * t2, GainType * G0, GainType * Gain);
extern Node *Best5OptMove(Node * t1, Node * t2, GainType * G0, GainType * Gain);
extern Node *BestKOptMove(Node * t1, Node * t2, GainType * G0, GainType * Gain);
extern int Between(const Node * ta, const Node * tb, const Node * tc);
extern int Between_SL(const Node * ta, const Node * tb, const Node * tc);
extern int Between_SSL(const Node * ta, const Node * tb, const Node * tc);
extern GainType BridgeGain(Node * s1, Node * s2, Node * s3, Node * s4,
                    Node * s5, Node * s6, Node * s7, Node * s8,
                    int Case6, GainType G);
extern Node **BuildKDTree(int Cutoff);
extern void ChooseInitialTour(void);
extern void Connect(Node * N1, int Max, int Sparse);
extern void CandidateReport(void);
extern void CreateCandidateSet(void);
extern void CreateDelaunayCandidateSet(void);
extern void CreateNearestNeighborCandidateSet(int K);
extern void CreateNNCandidateSet(int K);
extern void Create_POPMUSIC_CandidateSet(int K);
extern void CreateQuadrantCandidateSet(int K);
extern void eprintf(const char *fmt, ...);
extern int Excludable(Node * ta, Node * tb);
extern void Exclude(Node * ta, Node * tb);
extern GainType FindTour(void);
extern int FixedOrCommonCandidates(Node * N);
extern void Flip(Node * t1, Node * t2, Node * t3);
extern void Flip_SL(Node * t1, Node * t2, Node * t3);
extern void Flip_SSL(Node * t1, Node * t2, Node * t3);
extern int Forbidden(const Node * ta, const Node * tb);
extern void FreeCandidateSets(void);
extern void FreeSegments(void);
extern void FreeStructures(void);
extern int fscanint(FILE *f, int *v);
extern GainType Gain23(void);
extern void GenerateCandidates(int MaxCandidates, GainType MaxAlpha, int Symmetric);
extern double GetTime(void);
extern GainType GreedyTour(void);
extern void InitializeStatistics(void);
extern int IsBackboneCandidate(const Node * ta, const Node * tb);
extern int IsCandidate(const Node * ta, const Node * tb);
extern int IsCommonEdge(const Node * ta, const Node * tb);
extern int IsPossibleCandidate(Node * From, Node * To);
extern void KSwapKick(int K);
extern GainType LinKernighan(void);
extern void Make2OptMove(Node * t1, Node * t2, Node * t3, Node * t4);
extern void Make3OptMove(Node * t1, Node * t2, Node * t3, Node * t4, 
                  Node * t5, Node * t6, int Case);
extern void Make4OptMove(Node * t1, Node * t2, Node * t3, Node * t4,
                  Node * t5, Node * t6, Node * t7, Node * t8, 
                  int Case);
extern void Make5OptMove(Node * t1, Node * t2, Node * t3, Node * t4,
                  Node * t5, Node * t6, Node * t7, Node * t8,
                  Node * t9, Node * t10, int Case);
extern void MakeKOptMove(int K);
extern GainType MergeTourWithBestTour(void);
extern GainType MergeWithTourIPT(void);
extern GainType MergeWithTourGPX2(void);
extern GainType MergeWithTourCLARIST(void);
extern GainType Minimum1TreeCost(int Sparse);
extern void MinimumSpanningTree(int Sparse);
extern void NormalizeNodeList(void);
extern void NormalizeSegmentList(void);
extern void OrderCandidateSet(int MaxCandidates, 
                       GainType MaxAlpha, int Symmetric);
extern GainType PatchCycles(int k, GainType Gain);
extern void printff(char *fmt, ...);
extern void PrintParameters(void);
extern void PrintStatistics(void);
extern unsigned Random(void);
extern int ReadCandidates(int MaxCandidates);
extern int ReadEdges(int MaxCandidates);
extern char *ReadLine(FILE * InputFile);
extern void ReadParameters(void);
extern int ReadPenalties(void);
extern void ReadProblem(void);
extern void ReadTour(char * FileName, FILE ** File);
extern void RecordBestTour(void);
extern void RecordBetterTour(void);
extern Node *RemoveFirstActive(void);
extern void ResetCandidateSet(void);
extern void RestoreTour(void);
extern int SegmentSize(Node *ta, Node *tb);
extern GainType SFCTour(int CurveType);
extern void SolveCompressedSubproblem(int CurrentSubproblem, int Subproblems, 
                               GainType * GlobalBestCost);
extern void SolveDelaunaySubproblems(void);
extern void SolveKarpSubproblems(void);
extern void SolveKCenterSubproblems(void);
extern void SolveKMeansSubproblems(void);
extern void SolveRoheSubproblems(void);
extern void SolveSFCSubproblems(void);
extern int SolveSubproblem(int CurrentSubproblem, int Subproblems, 
                    GainType * GlobalBestCost);
extern void SolveSubproblemBorderProblems(int Subproblems, GainType * GlobalCost);
extern void SolveTourSegmentSubproblems(void);
extern void StoreTour(void);
extern void SRandom(unsigned seed);
extern void SymmetrizeCandidateSet(void);
extern void TrimCandidateSet(int MaxCandidates);
extern void UpdateStatistics(GainType Cost, double Time);
extern void WriteCandidates(void);
extern void WritePenalties(void);
extern void WriteTour(char * FileName, int * Tour, GainType Cost);
